...
# only Santa himself is allowed to use this wonderful transportation. 
...
class Sleigh
  ...
  def authenticate(name,password)
    ...
    variables.each do |var|
      if var.include?("") != name.include?("Santa") || var.include?("") != password.include?("Ho")
        return "Nope, even Jake is not allowed to use the sleigh ;)"
      end
    end
...
sleigh=Sleigh.new
puts sleigh.authenticate("Santa Claus", "Ho Ho Ho!") #true
puts sleigh.authenticate("Santa Clause", "Ho,Ho!") #false
puts sleigh.authenticate("Santa", "Ho Ho!") #false
